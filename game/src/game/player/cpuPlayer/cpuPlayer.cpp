...
constexpr float ADD_CPU_STATE_PROBABILITY = 10;		//行動状態の上昇確率
...
void CCpuPlayer::HitCalc(CObject* _hitObject)
{
	...
	{
		//m_targetObject = _hitObject;
		//m_cpuState = CPU_STATE_ATTACK;
	}
	//---------------------------------------------------------------------
}
...
void CCpuPlayer::ChangeCpuState()
{
	...
	for (int object_i = 0; object_i < m_FOV->GetObjectBufNum(); object_i++)
	{
		...
		case OBJECT_PLAYER:
			...
			break;
		case OBJECT_ITEM:
			//アイテムを持っている状態ならアイテムをカウントしない
			if (m_itemState == ITEM_STATE_HAVE)break;
			itemNum++;
